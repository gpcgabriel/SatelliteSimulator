...
class ComponentManager():
    ...
    @classmethod
    def initialize_simulator(cls, algorithms: list[callable]=[], num_executions: int=None, output=default_path_log, verbose: bool=False) -> None:
    ...
    @classmethod
    def create_services(cls, num_services, starts: list[int]=[0], demands: list[dict]=[{}], coordinates: list[tuple]=[()], services_durations: list[int]=[1]) -> None:
        ...
        services_durations = ComponentManager.complete_list_cyclically(services_durations, num_services)
        
        for i in range(0, num_services):
            cls.services.append(Service(starts[i], demands[i], coordinates[i], services_durations[i]))
